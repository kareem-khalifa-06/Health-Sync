...
async function insert(table, rows) {
  if (!rows || !rows.length) { console.log(`⚠  ${table} — no rows, skipping`); return; }
...
async function wipe() {
  ...
  const tables = [
    'notifications', 'payments', 'medical_records',
    'appointments', 'doctor_reviews', 'doctor_schedules',
    'doctors', 'patients', 'users', 'clinics',
  ];
...
function buildMap(collection) {
  const map = {};
  (db[collection] || []).forEach(r => { map[r.id] = uuid(); });
  return map;
}
...
async function main() {
  ...
  // ── 2. Users ─────────────────────────────────────────────────────
  // users.doctor_id / patient_id are plain uuid columns (no FK constraint)
  // so we can set them now even though doctors/patients aren't inserted yet.
  const userRows = (db.users || []).map(u => ({
    ...
    phone:      u.phone      || '',
    avatar:     u.avatar     || '',
    // back-references — may not exist for all users
  ...
  // ── 5. Doctor Schedules ──────────────────────────────────────────
  const scheduleRows = (db.doctorSchedules || []).map(s => ({
    id:        uuid(),
    clinic_id: clinicId,
    ...
    slots:     s.slots    || [],
  })).filter(s => s.doctor_id); // skip orphaned records
  await insert('doctor_schedules', scheduleRows);

  // ── 6. Doctor Reviews ────────────────────────────────────────────
  const reviewRows = (db.doctorReviews || []).map(r => ({
    id:               uuid(),
    clinic_id:        clinicId,
  ...
  // ── 7. Appointments ──────────────────────────────────────────────
  const appointmentRows = (db.appointments || []).map(a => ({
  ...
  // ── 8. Medical Records ───────────────────────────────────────────
  const recordRows = (db.medicalRecords || []).map(r => ({
    id:             recordMap[r.id],
    clinic_id:      clinicId,
    patient_id:     patientMap[r.patientId]       || null,
    doctor_id:      doctorMap[r.doctorId]         || null,
  ...
  // ── 10. Notifications ────────────────────────────────────────────
  const notificationRows = (db.notifications || []).map(n => ({
    id:             uuid(),
    clinic_id:      clinicId,
  ...
  // ── 2b. Create Supabase Auth accounts ───────────────────────────────────────
